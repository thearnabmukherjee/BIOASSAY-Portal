...
@Component({
	selector: 'app-plant-hpbinding',
	templateUrl: './plant-hpbinding.component.html',
	styleUrls: ['./plant-hpbinding.component.scss']
})
export class PlantHPBindingComponent implements OnInit 
{
	...
	{
		this.is_loading = true;
		this.userService.get_plant_hp_bindings().then((arrBinding)=>
		{
			this.arrRecords = [];
			for(let i = 0; i < arrUsers.length; ++i)
			{
				if(!arrUsers[i].roles.includes("hp"))
					continue;
				let arrPlants: { plant_id: number, plant_name: string }[] = [];
				for(let j = 0; j < arrBinding.length; ++j)
					if(arrBinding[j].hp_id == arrUsers[i].id)
	...
	change_assignment()
	{
		...
		this.userService.change_plant_assignment(this.selected_user_id, arr).then((res)=> 
		{
			this.is_loading = false; 
			if(res.isError)
				Message.show_message("Error", res.msg, true);
